...
def main():
    ...
    parser.add_argument('--model', type=str, default='gcl_neighsampler')
    ...
    if args.model == 'gcl_neighsampler': 
        activation = F.relu
        para_dict = gatv2_neighsampler_parameters
        encoder = Encoder(17, 32, activation, k=2).to(device)
        # print(dataset.num_features)
        model = Model(encoder, 32, 32, 0.4).to(device)
