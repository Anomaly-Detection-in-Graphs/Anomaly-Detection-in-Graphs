...
from gnn import test_auc as gnn_test
...
# def test_mini_loader_4training(layer_loader, model, data, y, evaluator, device, args, no_conv=False, use_mlp = False, the_model = None):
...
#     model_para.pop('lr')
#     model_para.pop('l2')   
#     detect_model = SAGE(in_channels = data.x.size(-1), out_channels = args.nlabels, encoder = the_model, **model_para).to(device)
...
#     with open('encoder_eval.txt', 'a') as f:
#         f.write(f'Edge Drop Rate 1: {the_model} \n')
...
def test_loader_mini4training(layer_loader, model, data, y, evaluator, device, args, no_conv=False, use_mlp = False, the_model = None):
    # data.y is labels of shape (N, ) 
    ...
    for epoch in range(1, 1200+1):
        ...
        total_loss = loss.item()
        eval_results, losses, out = gnn_test(detect_model, data, split_idx, evaluator, args, no_conv, is_loader = True)
        train_eval, valid_eval, test_eval = eval_results['train'], eval_results['valid'], eval_results['test']
        train_loss, valid_loss, test_loss = losses['train'], losses['valid'], losses['test']
        ...
        print(f'Best_Train: {100 * train_best_eval:.3f}%, '
                ...
                f'Best_Test: {100 * test_best_eval:.3f}%')
    with open('encoder_eval.txt', 'a') as f:
        f.write(f'The Used Encoder: {the_model} \n')
        f.write(f'Best_Train: {100 * train_best_eval:.3f}%, '
                f'Best_Valid: {100 * valid_best_eval:.3f}% '
                f'Best_Test: {100 * test_best_eval:.3f}% \n')
...
#     test(model, data.x, data.edge_index, data.y, final=True)
    ...
    # test_mini_loader_4training(layer_loader, model, data, data.y, Evaluator, device, args, no_conv=False, use_mlp = True, the_model = the_model)
    test_loader_mini4training(layer_loader, model, data, data.y, Evaluator, device, args, no_conv=False, use_mlp = False, the_model = the_model)
    
    
if __name__ == '__main__':
    device = 'cuda'
    model_list = ['SAGEConv', 'GraphConv', 'ResGatedGraphConv', 'TransformerConv', 'TAGConv', 'ARMAConv', 'SGConv', 'GENConv', 'ClusterGCNConv', 'SuperGATConv', 'GeneralConv', 'FiLMConv', 'GATConv', 'GATv2Conv', 'GCNConv']
    for the_model in model_list:
        # try:
        main(the_model)
        # except:
            # with open('encoder_eval.txt', 'a') as f:
            #     f.write(f'Error: {the_model} \n')