...
from torch_geometric.utils import dropout_adj, shuffle_node, dropout_node, add_random_edge
...
def train(epoch, train_loader, train_loader_oppo, model, model_oppo, data, train_idx, optimizer, optimizer_oppo, device, no_conv=False, is_DGraph = False, batch_size = 0, lam=0.3):
    model.train()
    model_oppo.train()
    ...
    alpha = 1.2
    loss_method = 'kl'
    for batch_size, n_id, adjs in train_loader_oppo:
...
#         # x_2, _ = shuffle_node(data.x[n_id])
        ...
        for edge_index, batch, size in adjs:
            ...
            x_1.append(drop_feature(data.x[n_id], drop_feature_rate_1))
            adjs_1.append((edge_index, batch, size))
            
            edge_index, edge_mask, node_mask = dropout_node(edge_index, p=drop_node_rate_2)
            # masked_indices = np.where(node_mask)[0]
            # masked_nodes = n_id[~np.isin(n_id, masked_indices)]
            x_2.append(drop_feature(data.x[n_id], drop_feature_rate_2))
            adjs_2.append((edge_index, batch, size))
        
        z1 = model(x_1, adjs_1)[:batch_size]
        z2 = model(x_2, adjs_2)[:batch_size] 
        z1_oppo = model_oppo(x_1, adjs_1)[:batch_size]
        z2_oppo = model_oppo(x_2, adjs_2)[:batch_size]
        loss = model.loss(z1, z2, batch_size=batch_size, cur_epoch = epoch)
        loss_oppo = model_oppo.loss(z1_oppo, z2_oppo, batch_size=batch_size, cur_epoch = epoch)
        loss_ = loss.clone().detach()
        
        if epoch > 1:
            if loss_method == 'kl':
                dl_diff = param_kl_difference(copy.deepcopy(model.state_dict()),copy.deepcopy(model_oppo.state_dict()), device) 
                optimizer_oppo.zero_grad()
                (alpha*loss_oppo - loss_  + lam * dl_diff).backward()
                optimizer_oppo.step()
            else:
                similarity = param_similarity(copy.deepcopy(model.state_dict()),copy.deepcopy(model_oppo.state_dict()), device) 
                optimizer_oppo.zero_grad()
                (alpha*loss_oppo - loss_  + (lam /similarity-1)**2).backward()
                optimizer_oppo.step()
...
#         # x_2, _ = shuffle_node(data.x[n_id])
        ...
        loss = model.loss(z1, z2, batch_size=batch_size, cur_epoch = epoch)
        loss_oppo = model_oppo.loss(z1_oppo, z2_oppo, batch_size=batch_size, cur_epoch = epoch)
        loss_oppo_ = loss_oppo.clone().detach()
        
        
        # loss = F.nll_loss(output, target) + (lam /similarity-1)**2
        
        if epoch > 1:
            if loss_method == 'kl':
                dl_diff = param_kl_difference(copy.deepcopy(model.state_dict()),copy.deepcopy(model_oppo.state_dict()), device) 
                optimizer.zero_grad()
                (alpha*loss - loss_oppo_  + lam * dl_diff).backward()
                optimizer.step()
            else:
                similarity = param_similarity(copy.deepcopy(model.state_dict()),copy.deepcopy(model_oppo.state_dict()), device) 
                optimizer.zero_grad()
                (alpha*loss - loss_oppo_  + (lam /similarity-1)**2).backward()
                optimizer.step()
        else:
            optimizer.zero_grad()
            loss.backward()
...
def param_similarity(t1,t2, device):
    similarity = []
    for p in t1:
        if len(t1[p].shape)==1:
          # print('1')
          t1[p]=torch.tensor([list(t1[p])])
        if len(t2[p].shape) == 1: # added
            t2[p]=torch.tensor([list(t2[p])])
        difference = torch.cosine_similarity(t1[p].to(device),t2[p].to(device)) # changed
        similarity.append(sum(difference)/len(difference))
    similarity = sum(similarity)/len(similarity)
    return similarity
        
def param_kl_difference(t1,t2, device):
    similarity = []
    ...
    for p in t1:
        ...
        if len(t2[p].shape) == 1: # added
            t2[p]=torch.tensor([list(t2[p])])
        similarity.append(kl_loss(t1[p].to(device),t2[p].to(device)))
    similarity = sum(similarity)/len(similarity)
    return similarity
...
#             y_discrete_pred = y_discrete_pred.argmax(axis=-1)

#             sorted_y = np.array(y_discrete_pred)
#             print('outoutout', np.sort(sorted_y))
#             y_pred_auc = out.detach().cpu().numpy()
#             y_pred_auc = y_pred_auc[:, 1]
#             y_test = the_data.y.detach().cpu().numpy()
#             y_test = y_test.reshape(-1, 1)
...
if __name__ == '__main__':
    ...
    torch.manual_seed(int(config['seed']//3))
    random.seed(int(12345//3))
    train_loader_oppo = NeighborSampler(data.edge_index, node_idx=train_idx, sizes=[3, 10], batch_size=batch_size, shuffle=True, num_workers=24)
    ...
    # encoder_oppo = Encoder(data.x.size(-1), num_hidden, activation,base_model=SAGEConv, k=num_layers).to(device)
    encoder_oppo = Encoder(data.x.size(-1), num_hidden, activation,base_model=base_model, k=num_layers).to(device)
    # print(dataset.num_features)
    model_oppo = Model(encoder_oppo, num_hidden, num_proj_hidden, tau, sim_filter_threshold = sim_filter_threshold).to(device)
    model_oppo.reset_parameters()
    optimizer_oppo = torch.optim.Adam(
        model_oppo.parameters(), lr=learning_rate, weight_decay=weight_decay)
    ...
    for epoch in range(1, num_epochs + 1):
        loss = train(epoch, train_loader, train_loader_oppo, model, model_oppo, data, train_idx, optimizer, optimizer_oppo, device, no_conv=False, batch_size = batch_size, is_DGraph = is_DGraph)
