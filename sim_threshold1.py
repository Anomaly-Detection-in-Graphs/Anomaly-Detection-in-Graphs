...
from models.gcl_neighsampler_Copy1 import Encoder, Model, drop_feature
...
# test_mini_loader_4training(layer_loader, model, data, data.y, Evaluator, device, args, rate_dic, no_conv=False, use_mlp = True, sim_filter_threshold = sim_filter_threshold, end_sim_filter_threshold = end_sim_filter_threshold, sim_dec_rate=sim_dec_rate)
def test_mini_loader_4training(layer_loader, model, data, y, evaluator, device, args, rate_dic, no_conv=False, use_mlp = False, sim_filter_threshold = None, end_sim_filter_threshold = None, sim_dec_rate=None):
    # data.y is labels of shape (N, ) 
    model.eval()
    eval_method = 'auc'
    # device = 'cpu'
...
#     model = model.to(device)
    ...
    with open('sim_threshold.txt', 'a') as f:
        f.write(f'Start Threshold: {sim_filter_threshold}, '
               f'End Threshold: {end_sim_filter_threshold}, '
               f'Decrease Rate:  {sim_dec_rate} \n')
        f.write(f'Best_Train: {100 * train_best_eval:.3f}%, '
                f'Best_Valid: {100 * valid_best_eval:.3f}% '
                f'Best_Test: {100 * test_best_eval:.3f}% \n')
...
def main(sim_filter_threshold, end_sim_filter_threshold, sim_dec_rate):
    ...
    rate_dic['drop_node_rate_1'] = config['drop_node_rate_1']
    rate_dic['drop_node_rate_2'] = config['drop_node_rate_2']
    tau = config['tau']
    ...
    if args.dataset != "DGraphFin":
        ...
        # sim_filter_threshold = config['threshold']
        # sim_filter_threshold, end_sim_filter_threshold, sim_dec_rate = config['start_threshold'], config['end_threshold'], config['decrease_rate']
    ...
    model = Model(encoder, num_hidden, num_proj_hidden, tau, sim_filter_threshold = sim_filter_threshold, end_sim_filter_threshold = end_sim_filter_threshold, sim_dec_rate=sim_dec_rate).to(device)
...
#     test(model, data.x, data.edge_index, data.y, final=True)
    ...
    print(sim_filter_threshold, end_sim_filter_threshold, sim_dec_rate)
    test_mini_loader_4training(layer_loader, model, data, data.y, Evaluator, device, args, rate_dic, no_conv=False, use_mlp = True, sim_filter_threshold = sim_filter_threshold, end_sim_filter_threshold = end_sim_filter_threshold, sim_dec_rate=sim_dec_rate)
    
if __name__ == '__main__':
    device = 'cuda'
    start_threshold = [0.0025, 0.002, 0.0015, 0.001, 0.0005, 0.0025, 0.002, 0.0015, 0.001, 0.0005]
    end_threshold = [0.0025/2.5, 0.002/2.5, 0.0015/2.5, 0.001/2.5, 0.0005/2, 0.0025/2, 0.002/2, 0.0015/2, 0.001/2, 0.0005/2]
    decrease_rate = [0.001, 0.001, 0.001, 0.001, 0.001, 0.003, 0.003, 0.003, 0.003, 0.003]
    for i in range(len(start_threshold)):
        main(start_threshold[i], end_threshold[i],decrease_rate[i])
