...
class NewEncoder(nn.Module):
    ...
    def forward(self, x: torch.Tensor, adjs, l = [1, 1]):
        for i, (edge_index, _, size) in enumerate(adjs):
            ...
            #emb = feat
            #print (K)
            x = self.activation(self.conv[i](x, edge_index, K))
    ...
    def inference(self, x_all, layer_loader, device):
        ...
        for i in range(self.k):
            ...
            for batch_size, n_id, adj in layer_loader:
                ...
                x_target = x[:size[1]]
                # x = self.conv[i]((x, x_target), edge_index, K)
                x = self.conv[i](x, edge_index, K)
                x = self.activation(x)
...
class NewGRACE(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, edge_index: torch.Tensor, R = [1,2], final = False) -> torch.Tensor:
        return self.encoder(x, edge_index, R)#, self.encoder2(x, edge_index)
